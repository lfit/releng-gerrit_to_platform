...
def check_cooldown(change_number: str, workflow_name: str) -> bool:
    ...
    try:
        ...
        try:
            fd = os.open(str(cooldown_file), os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o644)
            os.close(fd)
            ...
            if time_since_trigger < COOLDOWN_SECONDS:
                ...
                print(f"Cooldown active for workflow '{workflow_name}' on change {change_number}. "
...
def comment_added(
    ...
    if workflow_name:
        ...
        try:
            ...
            if dispatched == 0:
                print(f"No workflows found matching '{GHA_GENERIC_HANDLER}' for project {project}")
            ...
            print(f"Successfully dispatched {dispatched} workflow(s) for '{workflow_name}'")
