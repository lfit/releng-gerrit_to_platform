...
2. Parrotbot extracts the target bot (``@dependabot``) and the command
   (e.g., ``recreate``, ``rebase``).
3. It verifies the commenter is in an authorized Gerrit group.
...
- Only authorized Gerrit users (committers) can trigger the bot.
...
from gerrit_to_platform.config import get_boolean_setting, get_setting
...
def extract_github_pr_from_commit(
    ...
    try:
        ...
        if not match:
            log.info(
                "No GitHub-PR trailer found in change %s", change_number
            )
            return None
...
def handle_parrot_command(
    ...
    if not get_boolean_setting("parrotbot", "enabled", fallback=False):
    ...
    if not pr_info:
        ...
        post_gerrit_comment(
            gerrit_host,
            change_number,
            f"🦜 Parrotbot: Cannot find linked GitHub PR for this change. "
            f"Only changes created by GitHub2Gerrit (with GitHub-PR trailer) "
            f"are supported.",
        )
        return False
