...
class TestExtractGithubPrFromCommit:
    ...
    @patch("gerrit_to_platform.parrotbot.GerritRestAPI", create=True)
    def test_extracts_pr_info(self, mock_gerrit_cls):
        ...
        with patch(
            "gerrit_to_platform.parrotbot.GerritRestAPI",
            mock_gerrit_cls,
        ):
            # Need to patch the import inside the function
            import gerrit_to_platform.parrotbot as pb

            with patch.dict("sys.modules", {"pygerrit2": MagicMock()}):
                pb_module = pb
                # Directly mock the pygerrit2 import inside the function
                with patch.object(
                    pb_module,
                    "extract_github_pr_from_commit",
                    wraps=pb_module.extract_github_pr_from_commit,
                ):
                    pass

        # Simpler approach: call with mocked pygerrit2
...
def _call_extract_with_mock_gerrit(
    ...
    """Helper to call extract_github_pr_from_commit with mocked pygerrit2."""
    mock_api = MagicMock()
    ...
    mock_gerrit_module = MagicMock()
    mock_gerrit_module.GerritRestAPI.return_value = mock_api

    with patch.dict("sys.modules", {"pygerrit2": mock_gerrit_module}):
        ...
        importlib.reload(pb)
        return pb.extract_github_pr_from_commit(gerrit_host, change_number)
...
class TestPostGithubPrComment:
    ...
    def test_posts_comment_success(self):
        ...
        with patch.dict("sys.modules", {"ghapi.all": mock_ghapi_module, "ghapi": MagicMock()}):
            import importlib
...
class TestPostGerritComment:
    ...
    def test_posts_comment_success(self):
        ...
        with patch.dict("sys.modules", {"pygerrit2": mock_gerrit_module}):
            ...
            importlib.reload(pb)
            result = pb.post_gerrit_comment(
                "gerrit.example.org", "12345", "test message"
            )
...
class TestHandleParrotCommand:
    ...
    @patch(
        "gerrit_to_platform.parrotbot.get_boolean_setting", return_value=True
    )
    def test_successful_relay(
        ...
        """Full successful flow: parse → extract PR → relay → confirm."""
    ...
    @patch(
        "gerrit_to_platform.parrotbot.get_boolean_setting", return_value=False
    )
    ...
    def test_no_linked_pr_posts_failure(
        self, mock_enabled, mock_extract, mock_post_gerrit
    ):
    ...
    def test_bad_change_url_returns_false(self):
        ...
        with patch(
            "gerrit_to_platform.parrotbot.get_boolean_setting",
            return_value=True,
        ):
            result = handle_parrot_command(
                comment="@parrot @dependabot recreate",
