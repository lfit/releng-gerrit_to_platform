...
PATCH101_VERIFY = (
    "Dispatching workflow 'Verify', id 18525370 on "
    + "github:example/example-project for change 101 patch 2"
)
PATCH101_CHECK_MAIN = (
    "Dispatching workflow 'Check Main', id 17098575 on "
    + "github:example/example-project for change 101 patch 2"
...
def test_app(mocker):
    ...
    assert result.exit_code == 0
    assert PATCH1_VERIFY in result.stdout
    assert PATCH1_CHECK_MAIN in result.stdout

    result = runner.invoke(app, CHANGE101)
    assert result.exit_code == 0
    assert PATCH101_VERIFY in result.stdout
    assert PATCH101_CHECK_MAIN in result.stdout
