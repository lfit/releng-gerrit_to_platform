...
> **Update (December 2025):** GitHub raised the `workflow_dispatch` input
...
> for details. The constraint described below no longer blocks most use cases,
> but the architectural patterns documented here remain valid and this document
> retains the background context.

GitHub Actions enforced a hard limit of **10 inputs** for `workflow_dispatch` events. This constraint affected how we could trigger workflows from Gerrit events.
...
The `change-abandoned` Gerrit hook provides extra parameters:
...
## Solution: Fetch Extra Data from Gerrit API
...
│    - Passes 9 standard inputs to gerrit_to_platform             │
└─────────────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────────┐
│ 2. gerrit_to_platform Handler                                   │
│    - Creates inputs dict with 9 standard fields                 │
...
def change_abandoned(
    ...
    Note: the hook delivers abandoner, abandoner_username, and reason,
    but this handler does NOT forward them as workflow inputs due to
    GitHub's 10-input limit. GitHub2Gerrit fetches this information
    from the Gerrit REST API instead.
    """
    ...
    # 9 inputs - abandoner/reason fetched from API by GitHub2Gerrit
    inputs = {
...
def get_gerrit_change_details(
    ...
    Queries Gerrit with these options:
    - MESSAGES: Get all messages/comments
    - DETAILED_ACCOUNTS: Get full account details

    Return GerritChangeDetails with status, abandoner, reason, etc.
    """
...
def close_pr_with_status(
    ...
    if gerrit_change_details.status == "ABANDONED":
        ...
        comment_parts = [
            f"Closing this pull request because someone abandoned the "
            f"corresponding Gerrit change.",
            "",
...
✅ **Stays within GitHub's 10-input limit** - Uses 9 standard inputs

✅ **No information loss** - All abandoner/reason data remain available

✅ **Consistent with other workflows** - All Gerrit workflows use the same 9 inputs

✅ **More reliable** - Fetches the latest state from Gerrit API (source of truth)

✅ **Handles edge cases** - Works even when hook parameters go missing

✅ **Single source of truth** - Gerrit API is authoritative, not hook parameters

## Alternative: repository_dispatch

> **Note:** With the December 2025 increase to 25 inputs, most projects will
> not need `repository_dispatch`. This section remains as an option for edge
> cases that exceed even the new limit.
...
- [ ] Confirm workflow accepts 9 inputs
