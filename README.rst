...
the pattern. To trigger a single specific workflow, see the `Exact Workflow
...
All three workflows trigger for patchset-created events.

To trigger ONE specific workflow, enable exact matching in your
configuration file (``~/.config/gerrit_to_platform/gerrit_to_platform.ini``)::

    [workflow]
    exact_match = true

With exact matching enabled, workflows named precisely ``gerrit-verify.yaml``
or ``gerrit-merge.yaml`` will trigger. This prevents extra workflows
from triggering simultaneously.
